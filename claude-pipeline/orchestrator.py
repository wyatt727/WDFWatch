...
class UnifiedClaudePipeline:
    ...
    def _scrape_tweets(self, keywords: List[str], episode_id: str, tweets_file: str = None) -> List[Dict]:
        """Load tweets from file or scrape using keywords."""
        # Check if a specific tweets file was provided
        if tweets_file:
            ...
            if tweets_path.exists():
                ...
                console.print("[yellow]Please ensure transcripts/tweets.json exists or specify --tweets path[/yellow]")
                return []
        
        # Try to load from transcripts/tweets.json (default test data)
        transcripts_tweets = Path(__file__).parent.parent / "transcripts" / "tweets.json"
        ...
        # No mock tweets - require real data
