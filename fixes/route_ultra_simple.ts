...
export async function POST(
  ...
  try {
    ...
    // Run the FIXED bridge script with proper environment
    const scriptPath = path.join(process.cwd(), 'web', 'scripts', 'claude_pipeline_fixed.py');
    ...
    const fixedEnv = {
      ...
      PATH: [
        ...
        '/usr/sbin',
        '/sbin',
        `${homeDir}/.nvm/versions/node/v18.17.0/bin`,
        `${homeDir}/.nvm/versions/node/v20.10.0/bin`,
    ...
    console.log(`With PATH: ${fixedEnv.PATH}`);

    // Spawn the process with fixed environment
