...
class PreflightChecker:
    ...
    def check_environment(self) -> bool:
        ...
        # Check for API credentials - OAuth 2.0 style
        # For OAuth 2.0, we need API key, API secret, and access token
        # No access token secret is required for OAuth 2.0
        has_oauth2_credentials = all([
            os.getenv("TWITTER_API_KEY"),
            os.getenv("TWITTER_API_SECRET"),
            os.getenv("TWITTER_ACCESS_TOKEN")
        ])

        # Also check for WDFwatch OAuth 2.0 tokens
        has_wdfwatch_token = os.getenv("WDFWATCH_ACCESS_TOKEN")

        if not has_oauth2_credentials and not has_wdfwatch_token:
            self.errors.append("❌ Twitter API credentials not configured (OAuth 2.0)")
            return False
