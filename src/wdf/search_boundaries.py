...
class SearchBoundaryManager:
    ...
    def get_search_params(self, keyword: str, max_results: int = 10, 
        ...
        # Strategy: NEVER set both since_id and until_id to avoid Twitter API constraint violations
        # Priority order: 1) Search for new tweets, 2) Search for old tweets if no new available

        new_tweets_available = False

        # Check if we can search for new tweets since our newest (if within 7-day limit)
        if boundary.newest_id:
            # Twitter API v2 recent search only supports tweets from past 7 days
            # Check if the since_id is from a tweet older than 7 days
            if self._is_tweet_id_too_old(boundary.newest_id, max_age_days=7):
                logger.warning(
                    f"Stored since_id {boundary.newest_id} for '{keyword}' is older than 7 days, "
                    f"ignoring for Twitter API v2 recent search"
                )
                # Reset this boundary since it's too old
                del self.boundaries[keyword]
                params['search_type'] = 'initial'
                return params
            else:
                # Search for new tweets only
                params['since_id'] = boundary.newest_id
                params['search_type'] = 'new_only'
                new_tweets_available = True
                logger.info(
                    f"Searching '{keyword}' for new tweets since ID {boundary.newest_id} "
                    f"(last search: {age.days}d {age.seconds//3600}h ago)"
                )

        # Only search for older tweets if we're NOT searching for new tweets
        # AND we got full results last time (indicating more tweets might be available)
        if not new_tweets_available and boundary.results_count >= max_results and boundary.oldest_id:
            params['until_id'] = boundary.oldest_id
            params['search_type'] = 'old_only'
            logger.info(
                f"Searching '{keyword}' for older tweets before ID {boundary.oldest_id} "
                f"(previous search found full {boundary.results_count} results, likely more available)"
            )
        elif not new_tweets_available and boundary.results_count < max_results:
            logger.info(
    ...
    def _is_tweet_id_too_old(self, tweet_id: str, max_age_days: int = 7) -> bool:
        """
        Check if a Twitter snowflake ID represents a tweet older than max_age_days.

        Twitter snowflake IDs contain timestamp information. For Twitter API v2 recent search,
        we can only search tweets from the past 7 days.
        """
        try:
            # Twitter snowflake epoch: January 1, 2006 (different from Discord)
            # Format: timestamp (42 bits) + machine id (10 bits) + sequence (12 bits)
            tweet_id_int = int(tweet_id)

            # Extract timestamp from snowflake (first 42 bits, then add Twitter epoch)
            # Twitter epoch: 1142974214000 (milliseconds since Unix epoch for Jan 1, 2006)
            twitter_epoch_ms = 1142974214000
            timestamp_ms = (tweet_id_int >> 22) + twitter_epoch_ms

            # Convert to datetime
            tweet_datetime = datetime.fromtimestamp(timestamp_ms / 1000, tz=timezone.utc)

            # Check if older than max_age_days
            cutoff = datetime.now(timezone.utc) - timedelta(days=max_age_days)

            is_too_old = tweet_datetime < cutoff
            if is_too_old:
                logger.debug(f"Tweet ID {tweet_id} is from {tweet_datetime} (older than {max_age_days} days)")

            return is_too_old

        except (ValueError, OverflowError) as e:
            logger.warning(f"Could not parse tweet ID {tweet_id}: {e}, treating as too old")
            return True