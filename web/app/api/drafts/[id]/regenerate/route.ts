...
export async function POST(
  ...
  try {
    ...
    const claudePath = "/Users/pentester/.claude/local/claude"
    ...
    // Build the prompt components - no escaping needed since we're using spawn with direct args
    const promptBase = "Create a response to the provided tweet and DO NOT include any other text."
    const summaryRef = summaryLocation ? ` Refer to @${summaryLocation} for episode specific details.` : ""
    const promptEnd = " Ultrathink so your response is relevant, engaging, and original. Here is the tweet you must generate a response to:"
    const fullPrompt = `${promptBase}${summaryRef}${promptEnd} '${tweetText}'`
