...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: any = {
      ...
      // Note: WDF account filtering now happens at scraping stage (twitter_api_v2.py)
      // using -from: operators in search queries to prevent self-replies
    }
    ...
    // If filtering by specific tweet (by internal tweetId foreign key)
    if (tweetId) {
