...
export async function POST(
  ...
  try {
    ...
    if (!existsSync(filePath)) {
      // Only fall back to transcripts directory if this episode has no episode directory
      // This prevents showing files from other episodes
      if (!episode.claudeEpisodeDir && !episode.episodeDir) {
        // Try alternate locations for backwards compatibility - but only for episodes without directories
      ...
      return NextResponse.json({
        ...
        details: `Episode directory: ${episodeDirPath}`
      }, { status: 404 })
    }
