...
import { z } from 'zod';
import { writeFile, mkdir, copyFile, access, chmod } from 'fs/promises';
...
// Helper function to fix file/directory permissions for cross-user access
async function fixEpisodePermissions(episodeDir: string) {
  try {
    console.log(`[Permission Fix] Setting up permissions for ${episodeDir}`);

    // Set directory permissions to 777 (rwxrwxrwx) for maximum compatibility
    // This allows both container user (1001) and host user (debian) to write
    await chmod(episodeDir, 0o777);
    console.log(`[Permission Fix] Set directory permissions to 777 (full access)`);

    // Also set permissions for any existing files
    const { readdir, chmod: chmodFile, stat } = await import('fs/promises');
    ...
    try {
      ...
      for (const file of files) {
        ...
        if (stats.isDirectory()) {
          ...
          await fixEpisodePermissions(filePath);
        } else {
          // Set file permissions to 666 (rw-rw-rw-)
          await chmod(filePath, 0o666);
        }
      }
      console.log(`[Permission Fix] Set permissions for ${files.length} files/directories`);
    } catch (fileError) {
      console.log(`[Permission Fix] No files to update or error updating files: ${fileError.message}`);
    }

  } catch (error) {
    console.warn(`[Permission Fix] Failed to fix permissions for ${episodeDir}:`, error);
    // Don't throw - just log the warning so episode creation doesn't fail
  }
}
...
async function setupEpisodeDirectory(episodeTitle: string, transcript: string, videoUrl?: string, pipelineType: 'claude' | 'legacy' = 'legacy') {
  ...
  const episodesBaseDir = process.env.EPISODES_DIR || join(process.cwd(), '..', 'claude-pipeline', 'episodes');
  ...
  if (pipelineType === 'claude') {
    ...
    try {
      ...
      throw new Error(`Episode with title "${episodeTitle}" already exists. Please choose a different title.`);
    } catch (error: any) {
      // If it's our custom error, re-throw it
      if (error.message?.includes('already exists')) {
        throw error;
      }
      // Otherwise, directory doesn't exist, we can create it
    }
    
    // Create Claude pipeline structure
    await mkdir(episodeDir, { recursive: true });

    // IMMEDIATELY fix permissions for the new episode directory
    await fixEpisodePermissions(episodeDir);
    console.log(`[Episode Setup] Fixed permissions for new episode: ${episodeDir}`);
    ...
    // Fix permissions AGAIN after all files are written
    await fixEpisodePermissions(episodeDir);
    console.log(`[Episode Setup] Re-fixed permissions after writing all files`);
  ...
  // Note: transcripts directory is shared, don't change its permissions
  ...
  // Note: Permission fixing for Claude episodes is already handled twice above:
  // 1. Right after creating the directory
  // 2. After writing all files
  // No need for a third call here
...
export async function POST(req: NextRequest) {
  try {
    ...
    let episodeDirName, claudeEpisodeDir;
    try {
      const result = await setupEpisodeDirectory(
        title,
        transcript,
        videoUrl,
        pipelineType
      );
      episodeDirName = result.episodeDirName;
      claudeEpisodeDir = result.claudeEpisodeDir;
    } catch (error: any) {
      console.error('Error creating episode directory:', error);

      // Delete the episode from database since we couldn't create its directory
      ...
      return NextResponse.json(
        {
          error: 'Failed to create episode directory',
