        ...
        <Tabs defaultValue="twitter" className="space-y-4">
          ...
          <TabsContent value="twitter" className="space-y-4">
              ...
              <AlertDescription>
                <div className="space-y-2">
                  ...
                  <div className="text-xs space-y-1 mt-2">
                    <p className="font-semibold">Required credentials:</p>
                    <p>• <strong>For reading only:</strong> API Key, API Secret, Bearer Token</p>
                    <p>• <strong>For posting (OAuth 2.0):</strong> API Key, API Secret, Access Token</p>
                    <p>• <strong>For posting (OAuth 1.0a):</strong> All fields including Access Token Secret</p>
                  </div>
                </div>
              </AlertDescription>
            </Alert>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="twitter-access-token">
                  Access Token <span className="text-xs text-muted-foreground">(OAuth 2.0 or OAuth 1.0a)</span>
                </Label>
                <div className="flex gap-2">
                  <Input
                    id="twitter-access-token"
                    type={showKeys['twitter-access-token'] ? 'text' : 'password'}
                    value={formData.twitter?.accessToken || ''}
                    onChange={(e) => handleInputChange('twitter', 'accessToken', e.target.value)}
                    placeholder={apiKeys?.twitter?.accessTokenSet ? 'Enter new token to update' : 'Enter access token'}
                  />
                  <Button
                    type="button"
                    variant="outline"
                    size="icon"
                    onClick={() => toggleShowKey('twitter-access-token')}
                  >
                    {showKeys['twitter-access-token'] ? <EyeOffIcon className="h-4 w-4" /> : <EyeIcon className="h-4 w-4" />}
                  </Button>
                </div>
                {apiKeys?.twitter?.accessTokenSet && (
                  <p className="text-sm text-muted-foreground flex items-center gap-1">
                    <CheckCircle2 className="h-3 w-3 text-green-500" />
                    Currently set
                  </p>
                ...
                <p className="text-xs text-muted-foreground">
                  For OAuth 2.0: The token from your OAuth 2.0 flow. For OAuth 1.0a: The user access token.
                </p>
              </div>

              <div className="space-y-2">
                <Label htmlFor="twitter-access-token-secret">
                  Access Token Secret <span className="text-xs text-muted-foreground">(OAuth 1.0a only)</span>
                </Label>
                <div className="flex gap-2">
                  <Input
                    id="twitter-access-token-secret"
                    type={showKeys['twitter-access-token-secret'] ? 'text' : 'password'}
                    value={formData.twitter?.accessTokenSecret || ''}
                    onChange={(e) => handleInputChange('twitter', 'accessTokenSecret', e.target.value)}
                    placeholder={apiKeys?.twitter?.accessTokenSecretSet ? 'Enter new secret to update' : 'Enter access token secret'}
                  />
                  <Button
                    type="button"
                    variant="outline"
                    size="icon"
                    onClick={() => toggleShowKey('twitter-access-token-secret')}
                  >
                    {showKeys['twitter-access-token-secret'] ? <EyeOffIcon className="h-4 w-4" /> : <EyeIcon className="h-4 w-4" />}
                  </Button>
                </div>
                {apiKeys?.twitter?.accessTokenSecretSet && (
                  <p className="text-sm text-muted-foreground flex items-center gap-1">
                    <CheckCircle2 className="h-3 w-3 text-green-500" />
                    Currently set
                  </p>
                ...
                <p className="text-xs text-muted-foreground">
                  Only needed for OAuth 1.0a. Leave empty if using OAuth 2.0.
                </p>
              </div>
            </div>
          </TabsContent>
