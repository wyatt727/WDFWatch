...
class WebUIBridge:
    ...
    def __init__(self):
        ...
        self.web_url = os.getenv("WEB_URL", "http://localhost:8888")
    ...
    def sync_tweets(self, tweets: List[Dict]) -> None:
        ...
        # Get episode_id from environment if set (for episode-specific runs)
        episode_id = os.getenv('WDF_CURRENT_EPISODE_ID')
        logger.info(f"WDF_CURRENT_EPISODE_ID from environment: '{episode_id}'")

        if episode_id:
            try:
                episode_id = int(episode_id)
                logger.info(f"✅ Using episode_id {episode_id} for tweet association")
            except ValueError:
                episode_id = None
                logger.warning(f"❌ Invalid episode_id in environment: '{os.getenv('WDF_CURRENT_EPISODE_ID')}'")

        try:
            with self.connection.cursor() as cursor:
                for tweet in tweets:
                    # Handle user field - could be string or dict
                    if isinstance(tweet.get('user'), dict):
                        author_handle = tweet['user'].get('username', 'unknown')
                    else:
                        author_handle = tweet.get('user', 'unknown')

                    # Handle created_at field - could be string, datetime, or missing
                    ...
                    cursor.execute("""
                        INSERT INTO tweets (
                            ...
                            text_preview, created_at, status, episode_id, updated_at
                        ) VALUES (%s, %s, %s, %s, %s, %s, %s, CURRENT_TIMESTAMP)
                        ON CONFLICT (twitter_id) DO UPDATE
                        SET full_text = EXCLUDED.full_text,
                            text_preview = EXCLUDED.text_preview,
                            episode_id = COALESCE(EXCLUDED.episode_id, tweets.episode_id),
                ...
                if episode_id:
                    logger.info(f"Synced {len(tweets)} tweets to database for episode {episode_id}")
                else:
                    logger.info(f"Synced {len(tweets)} tweets to database (no episode association)")
...
def get_keywords_if_web_mode(episode_id: Optional[str] = None) -> Optional[List[str]]:
    ...
    Returns:
        List of keyword strings if in web mode, None otherwise (including when no keywords found)
    """
    if os.getenv("WDF_WEB_MODE", "false").lower() == "true":
        bridge = get_bridge()
        keywords = bridge.get_enabled_keywords(episode_id)
        # Convert to simple list of strings, but return None if empty to allow fallback
        if keywords:
            return [kw['keyword'] for kw in keywords]
        else:
            # Return None when no keywords found so scraping task can fall back to file-based loading
            return None
    return None
